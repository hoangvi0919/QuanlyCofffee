...
public class ManagementCart {
    ...
    public void insertFood(FoodDomain item) {
        ...
        for (int i = 0; i < listFood.size(); i++) {
            if (listFood.get(i).getMon().equals(item.getMon())) {
                existAlready = true;
                n = i;
                break;
            }
    ...
    public void MinusNumerFood(ArrayList<FoodDomain> listfood, int position, ChangeNumberItemListener changeNumberItemsListener) {
    ...
    public int getTotalFee() {
        ...
        for (int i = 0; i < listFood2.size(); i++) {
            fee = fee + (listFood2.get(i).getGia() * listFood2.get(i).getNumberInCart());
        }
        return fee;
    }
